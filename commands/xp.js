...
module.exports = {
  name: 'xp',
  aliases: ['experience'],
  description: 'Zeigt deinen XP-Status an',
  ...
  async execute({ m, send, args }) {
    ...
    //    numerator = currentXp * 100, denominator = maxXp
...
╠══════════════════════╣
║ Rank       : ${user.role}
║ XP         : ${this.formatNumber(user.exp)}
║ Level      : ${user.level}
║ Fortschritt: ${this.formatNumber(currentXp)} / ${this.formatNumber(maxXp)} XP
║ Fehlen noch: ${this.formatNumber(xpNeeded)} XP
