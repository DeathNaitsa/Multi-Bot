// commands/unmute.js
const { isBotAdmin, isUserAdmin } = require('../utils/groupfunctions');
const { loadChatById } = require('../db/chats');
const { t } = require('../utils/i18n');

module.exports = {
  name: 'unmute',
  get description() {
    return t('unmute_description', 'de');
  },
  async execute({ m, args, send, platformApi }) {
    const chat = await loadChatById(m.chat);
    const lang = chat?.language || 'de';
    if (!m.chat || m.chat === m.sender) return send({ chat: m.chat, text: t('not_group', lang) });
    const botIsAdmin = await isBotAdmin(m, platformApi);
    if (!botIsAdmin) return send({ chat: m.chat, text: t('bot_not_admin', lang) });
    const userIsAdmin = await isUserAdmin(m, m.sender, platformApi);
    if (!userIsAdmin) return send({ chat: m.chat, text: t('unmute_not_admin', lang) });
    const toUnmute = args[0];
    if (m.platform === 'discord') {
      try {
        const guild = await platformApi.guilds.fetch(m.guildId);
        const member = await guild.members.fetch(toUnmute.replace(/[^0-9]/g, ''));
        await member.timeout(null); // Timeout aufheben
        return send({ chat: m.chat, text: t('unmute_success', lang) });
      } catch (e) { return send({ chat: m.chat, text: t('unmute_error', lang, { error: e.message }) }); }
    } else if (m.platform === 'telegram') {
      try {
        await platformApi.restrictChatMember(m.chat, toUnmute.replace(/[^0-9]/g, ''), { can_send_messages: true });
        return send({ chat: m.chat, text: t('unmute_success_telegram', lang) });
      } catch (e) { return send({ chat: m.chat, text: t('unmute_error', lang, { error: e.message }) }); }
    } else {
      return send({ chat: m.chat, text: t('unmute_not_supported', lang) });
    }
  }
};
