...
async function findByPlatform(platform, platformId) {
      ...
      case 'whatsapp':
        if (Array.isArray(u.whatsappNumbers)) {
          for (const enc of u.whatsappNumbers) {
            try {
              if (decrypt(enc) === platformId) return u;
            } catch {}
          }
        }
        break;
...
async function createOrReuseUser(platform, platformId, initialFields = {}) {
 ...
 const newUser = {
    id: newId,
    serialNumber,
    whatsappNumbers: platform === 'whatsapp' ? [encrypt(platformId)] : [],
  ...
  switch (platform) {
    case 'whatsapp':
      // Für WhatsApp verschlüsseln wir die Telefonnummer
...
async function registerUserWithData(platform, platformId, oldUserData = {}) {
